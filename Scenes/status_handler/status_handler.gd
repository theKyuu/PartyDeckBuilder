...
func add_status(status: Status) -> void:
	...
	if not _has_status(status.id):
		...
		new_status_ui.status.intitialize_status(status_owner)
		return
	
	# If it's already appolied and unique, ignore
	if not status.can_expire and not stackable:
		return
	
	# If it's duration-stackable, add to it
