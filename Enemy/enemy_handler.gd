...
func _ready() -> void:
	...
	Events.player_hand_drawn.connect(_on_player_hand_drawn) # This is to update enemy intents after all "start of turn effects" - Should maybe trigger whenever a player status changes instead?
...
func _on_enemy_statuses_applied(type: Status.Type, enemy: Enemy) -> void:
	# Called once Start of Turn or End of Turn status effects have been applied. 
	# Then either have the enemy do their turn or move on to the next one.
	match type:
